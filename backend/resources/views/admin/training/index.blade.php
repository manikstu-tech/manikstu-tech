...
@section('content')
<div class="page-header"><div><h1 class="page-title">Training Programs</h1></div><a href="{{ route('admin.training.create') }}" class="btn btn-primary"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14"/><path d="M5 12h14"/></svg> Add Program</a></div>
@if(session('success'))<div class="alert alert-success">{{ session('success') }}</div>@endif
<x-admin.table :headers="['Title', 'Order', 'Status', 'Actions']">
    @foreach($programs as $p)<tr><td><strong>{{ $p->title }}</strong></td><td>{{ $p->order }}</td><td><x-admin.badge :type="$p->is_active ? 'green' : 'red'" dot>{{ $p->is_active ? 'Active' : 'Inactive' }}</x-admin.badge></td><td><div class="table-actions"><a href="{{ route('admin.training.edit', $p) }}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></a><button type="button" class="btn-delete" onclick="openModal('deleteModal','{{ route('admin.training.destroy', $p) }}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></div></td></tr>@endforeach
</x-admin.table>
