                ...
                <x-admin.translation-tabs>
                    ...
                    @foreach(['hi'=>'हिन्दी','bn'=>'বাংলা','ta'=>'தமிழ்','te'=>'తెలుగు','mr'=>'मराठी','gu'=>'ગુજરાતી','kn'=>'ಕನ್ನಡ','ml'=>'മലയാളം','or'=>'ଓଡ଼ିଆ','ja'=>'日本語','de'=>'Deutsch','fr'=>'Français','es'=>'Español'] as $code => $label)
                        <div class="translation-panel" data-locale="{{ $code }}">
                            ...
                            <x-admin.form-field label="Name" name="name_{{ $code }}" placeholder="Translation in {{ $label }}" />
                            <x-admin.form-field label="Description" name="description_{{ $code }}" type="textarea" :rows="4" placeholder="Translation in {{ $label }}" />
                        </div>
                    @endforeach
                </x-admin.translation-tabs>
            </div>
        ...
        <div class="form-side">
            <div class="form-card">
                ...
                <div style="padding:18px;">
                    ...
                    <x-admin.form-field label="Order" name="order" type="number" :value="$product->order ?? 0" />
                    <x-admin.form-field label="Featured" name="is_featured" type="toggle" :value="$product->is_featured ?? false" />
                    <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$product->is_active ?? true" />
                    <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;">{{ isset($product) ? 'Update' : 'Create' }}</button>
                </div>
            </div>
            <div class="form-card">
                <div class="card-header" style="padding:14px 18px;border-bottom:1px solid #E5E5E5;font-weight:700;font-size:14px;">Image</div>
                <div style="padding:18px;">
                    <x-admin.upload name="image" label="Product Image" :current="$product->image ?? null" />
                </div>
            </div>
        </div>
    </div>
