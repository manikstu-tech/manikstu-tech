...
<div class="filter-bar">
    <form method="GET" class="filter-form">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search posts..." class="filter-input">
    ...
    @foreach($posts as $post)
        <tr>
            <td>
                ...
                @if($post->is_featured)<br><small style="color:#C4952A;">Featured</small>@endif
            </td>
            <td>{{ $post->category->name ?? '—' }}</td>
            <td><x-admin.badge :type="$post->is_published ? 'green' : 'gold'" dot>{{ $post->is_published ? 'Published' : 'Draft' }}</x-admin.badge></td>
            <td>{{ $post->published_at?->format('M d, Y') ?? $post->created_at->format('M d, Y') }}</td>
            <td>
                <div class="table-actions">
                    <a href="{{ route('admin.blog.edit', $post) }}" title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></a>
                    <button type="button" class="btn-delete" onclick="openModal('deleteModal','{{ route('admin.blog.destroy', $post) }}')" title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
                </div>
            </td>
        </tr>
    @endforeach
...
<div class="table-pagination">{{ $posts->links() }}</div>
