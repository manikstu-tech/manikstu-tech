...
@section('content')
<div class="page-header"><h1 class="page-title">Edit Partner</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ route('admin-preview.partners.update', $partner) }}">
    @csrf @method('PUT')
    <div class="form-card">
        ...
        <x-admin.form-field label="Category" name="category" :value="$partner->category" />
        <x-admin.form-field label="Order" name="order" type="number" :value="$partner->order" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$partner->is_active" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.partners.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">Update</button></div>
</form>
