...
@section('content')
<div class="page-header"><h1 class="page-title">{{ isset($partner) ? 'Edit Partner' : 'Add Partner' }}</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ isset($partner) ? route('admin-preview.partners.update', $partner) : route('admin-preview.partners.store') }}">
    @csrf @if(isset($partner)) @method('PUT') @endif
    <div class="form-card">
        <x-admin.form-field label="Name" name="name" :value="$partner->name ?? ''" required />
        <x-admin.form-field label="Website URL" name="website_url" type="url" :value="$partner->website_url ?? ''" />
        <x-admin.form-field label="Category" name="category" :value="$partner->category ?? ''" help="e.g. incubation, government, corporate, ngo" />
        <x-admin.form-field label="Order" name="order" type="number" :value="$partner->order ?? 0" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$partner->is_active ?? true" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.partners.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($partner) ? 'Update' : 'Create' }}</button></div>
</form>
