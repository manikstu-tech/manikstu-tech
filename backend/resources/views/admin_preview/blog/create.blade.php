...
<form method="POST" action="{{ isset($post) ? route('admin-preview.blog.update', $post) : route('admin-preview.blog.store') }}">
    @csrf
                ...
                <x-admin.translation-tabs>
                    ...
                    @foreach(['hi'=>'हिन्दी','bn'=>'বাংলা','ta'=>'தமிழ்','te'=>'తెలుగు','mr'=>'मराठी','gu'=>'ગુજરાતી','kn'=>'ಕನ್ನಡ','ml'=>'മലയാളം','or'=>'ଓଡ଼ିଆ','ja'=>'日本語','de'=>'Deutsch','fr'=>'Français','es'=>'Español'] as $code => $label)
                        <div class="translation-panel" data-locale="{{ $code }}">
                            ...
                            <x-admin.form-field label="Title" name="title_{{ $code }}" placeholder="Translation in {{ $label }}" />
                            <x-admin.form-field label="Excerpt" name="excerpt_{{ $code }}" type="textarea" :rows="2" placeholder="Translation in {{ $label }}" />
                            <x-admin.form-field label="Content" name="content_{{ $code }}" type="textarea" :rows="6" placeholder="Translation in {{ $label }}" />
                        </div>
                    @endforeach
                </x-admin.translation-tabs>
            </div>
