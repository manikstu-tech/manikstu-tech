...
<div class="page-header">
    ...
    <div class="header-actions">
        <a href="{{ route('admin-preview.products.index') }}" class="btn btn-light">← Back</a>
        <form method="POST" action="{{ route('admin-preview.products.togglePublish', $product) }}">
            @csrf @method('PUT')
            <button type="submit" class="btn btn-light">{{ $product->is_active ? 'Unpublish' : 'Publish' }}</button>
        </form>
        <a href="{{ route('admin-preview.products.edit', $product) }}" class="btn btn-primary">Edit Product</a>
    </div>
</div>
