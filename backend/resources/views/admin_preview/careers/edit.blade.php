...
@section('content')
<div class="page-header"><h1 class="page-title">Edit Opening</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ route('admin-preview.careers.update', $job) }}">
    @csrf @method('PUT')
    <div class="form-card">
        ...
        <x-admin.form-field label="Type" name="type" type="select" :options="['full_time'=>'Full Time','part_time'=>'Part Time','contract'=>'Contract','internship'=>'Internship']" :value="$job->type" required />
        ...
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$job->is_active" />
    </div>
    <div class="form-actions">
        <a href="{{ route('admin-preview.careers.index') }}" class="btn btn-secondary">Cancel</a>
