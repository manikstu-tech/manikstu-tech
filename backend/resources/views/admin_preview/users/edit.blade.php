...
@section('content')
<div class="page-header"><h1 class="page-title">Edit User</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ route('admin-preview.users.update', $user) }}">
    @csrf @method('PUT')
    <div class="form-card">
    ...
    <div class="form-actions"><a href="{{ route('admin-preview.users.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">Update</button></div>
</form>
