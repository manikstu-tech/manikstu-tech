...
<form method="POST" action="{{ isset($user) ? route('admin-preview.users.update', $user) : route('admin-preview.users.store') }}">
    ...
    <div class="form-actions"><a href="{{ route('admin-preview.users.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($user) ? 'Update' : 'Create' }}</button></div>
</form>
