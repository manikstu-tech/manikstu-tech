...
@section('content')
<div class="page-header"><h1 class="page-title">Edit Program</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ route('admin-preview.training.update', $program) }}">
    @csrf @method('PUT')
    <div class="form-card">
        ...
        <x-admin.form-field label="Icon" name="icon" :value="$program->icon" />
        <x-admin.form-field label="Order" name="order" type="number" :value="$program->order" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$program->is_active" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.training.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">Update</button></div>
</form>
