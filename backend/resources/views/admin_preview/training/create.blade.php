...
@section('content')
<div class="page-header"><h1 class="page-title">{{ isset($program) ? 'Edit Program' : 'Add Program' }}</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ isset($program) ? route('admin-preview.training.update', $program) : route('admin-preview.training.store') }}">
    @csrf @if(isset($program)) @method('PUT') @endif
    <div class="form-card">
        <x-admin.form-field label="Title" name="title" :value="$program->title ?? ''" required />
        <x-admin.form-field label="Description" name="description" type="textarea" :value="$program->description ?? ''" :rows="4" />
        <x-admin.form-field label="Icon" name="icon" :value="$program->icon ?? ''" help="Icon class or SVG name" />
        <x-admin.form-field label="Order" name="order" type="number" :value="$program->order ?? 0" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$program->is_active ?? true" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.training.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($program) ? 'Update' : 'Create' }}</button></div>
</form>
