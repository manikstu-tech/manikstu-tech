...
@section('content')
<div class="page-header"><h1 class="page-title">{{ isset($order) ? 'Edit Order' : 'New Order' }}</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ isset($order) ? route('admin-preview.orders.update', $order) : route('admin-preview.orders.store') }}">
    @csrf @if(isset($order)) @method('PUT') @endif
    <div class="form-card">
        @if(!isset($order))<x-admin.form-field label="Total (₹)" name="total" type="number" :value="$order->total ?? ''" required />@endif
        <x-admin.form-field label="Status" name="status" type="select" :options="['pending'=>'Pending','confirmed'=>'Confirmed','shipped'=>'Shipped','delivered'=>'Delivered','cancelled'=>'Cancelled']" :value="$order->status ?? 'pending'" required />
        <x-admin.form-field label="Payment Status" name="payment_status" type="select" :options="['unpaid'=>'Unpaid','paid'=>'Paid','refunded'=>'Refunded']" :value="$order->payment_status ?? 'unpaid'" required />
    ...
    <div class="form-actions"><a href="{{ route('admin-preview.orders.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($order) ? 'Update' : 'Create' }}</button></div>
</form>
