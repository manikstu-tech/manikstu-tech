...
@section('content')
<div class="page-header"><h1 class="page-title">{{ isset($testimonial) ? 'Edit Testimonial' : 'Add Testimonial' }}</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ isset($testimonial) ? route('admin-preview.testimonials.update', $testimonial) : route('admin-preview.testimonials.store') }}">
    ...
    <div class="form-card">
        ...
        <x-admin.form-field label="Order" name="order" type="number" :value="$testimonial->order ?? 0" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$testimonial->is_active ?? true" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.testimonials.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($testimonial) ? 'Update' : 'Create' }}</button></div>
</form>
