...
<form method="POST" action="{{ route('admin-preview.categories.update', $category) }}">
    @csrf
    @method('PUT')
    ...
    <div class="form-card">
        ...
        <x-admin.form-field label="Type" name="type" type="select" :options="['blog' => 'Blog', 'product' => 'Product', 'gallery' => 'Gallery', 'training' => 'Training']" :value="$category->type" required />
        <x-admin.form-field label="Order" name="order" type="number" :value="$category->order" />
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$category->is_active" />
    </div>

    <div class="form-actions">
        <a href="{{ route('admin-preview.categories.index') }}" class="btn btn-secondary">Cancel</a>
