...
<form method="POST" action="{{ isset($initiative) ? route('admin-preview.awareness.update', $initiative) : route('admin-preview.awareness.store') }}">
    ...
    <div class="form-actions"><a href="{{ route('admin-preview.awareness.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($initiative) ? 'Update' : 'Create' }}</button></div>
</form>
