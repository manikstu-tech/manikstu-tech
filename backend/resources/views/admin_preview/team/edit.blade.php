...
@section('content')
<div class="page-header"><h1 class="page-title">Edit Member</h1></div>
@if($errors->any())<div class="alert alert-danger">@foreach($errors->all() as $e) {{ $e }} @endforeach</div>@endif
<form method="POST" action="{{ route('admin-preview.team.update', $member) }}">
    @csrf @method('PUT')
    <div class="form-grid">
        ...
        <div class="form-side">
            <div class="form-card"><div class="card-header" style="padding:14px 18px;border-bottom:1px solid #E5E5E5;font-weight:700;font-size:14px;">Settings</div><div style="padding:18px;">
                <x-admin.form-field label="Order" name="order" type="number" :value="$member->order" />
                <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$member->is_active" />
            ...
            <div class="form-card"><div class="card-header" style="padding:14px 18px;border-bottom:1px solid #E5E5E5;font-weight:700;font-size:14px;">Photo</div><div style="padding:18px;">
                <x-admin.upload name="image" label="Photo" :current="$member->image" />
            </div></div>
        </div>
    </div>
</form>
