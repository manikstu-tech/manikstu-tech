...
<form method="POST" action="{{ route('admin-preview.customers.update', $customer) }}">
    @csrf @method('PUT')
    <div class="form-card">
        ...
        <x-admin.form-field label="Active" name="is_active" type="toggle" :value="$customer->is_active" />
    </div>
    <div class="form-actions"><a href="{{ route('admin-preview.customers.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">Update</button></div>
</form>
