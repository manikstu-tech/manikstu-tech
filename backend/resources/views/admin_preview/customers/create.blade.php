...
<form method="POST" action="{{ isset($customer) ? route('admin-preview.customers.update', $customer) : route('admin-preview.customers.store') }}">
    ...
    <div class="form-actions"><a href="{{ route('admin-preview.customers.index') }}" class="btn btn-secondary">Cancel</a><button type="submit" class="btn btn-primary">{{ isset($customer) ? 'Update' : 'Create' }}</button></div>
</form>
